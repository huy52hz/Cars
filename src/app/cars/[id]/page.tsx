...
import {
  Fuel, Calendar, Palette, DollarSign, Gauge, Car, MapPin, Tag, Clock, PackageCheck, Edit, Trash2, ArrowLeft, ShoppingCart, // Thêm Trash2 vì bạn cần nó cho nút Xóa
} from 'lucide-react';
            ...
            <div className="max-w-7xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
              ...
              <div className="md:flex">
                <div className="md:w-1/2 p-4">
                  {car.images && car.images.length > 0 ? (
                    <div className="relative w-full aspect-video rounded-lg overflow-hidden mb-4 border border-gray-200">
                      <Image src={car.images[0]} alt={car.name} fill style={{ objectFit: 'cover' }} className="rounded-lg" priority sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 50vw" />
                    </div>
                  ) : (
                  ...
                  {car.images && car.images.length > 1 && (
                    <div className="grid grid-cols-4 gap-2">
                      {car.images.slice(1, 5).map((img, index) => (
                        <div key={index} className="relative aspect-square rounded-md overflow-hidden cursor-pointer hover:opacity-75 transition-opacity border border-gray-200">
                          <Image src={img} alt={`${car.name} - ảnh phụ ${index + 1}`} fill style={{ objectFit: 'cover' }} sizes="25vw" />
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="md:w-1/2 p-6 md:p-8">
                  ...
                  <p className="text-xl text-blue-600 font-semibold mb-6">{formatPrice(car.price)}</p>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 text-gray-700">
                    <div className="flex items-center gap-2">
                      <Tag className="w-5 h-5 text-gray-500" />
                      <span>Thương hiệu: <span className="font-medium">{car.brand}</span></span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Car className="w-5 h-5 text-gray-500" />
                      <span>Dòng xe: <span className="font-medium">{car.category}</span></span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Calendar className="w-5 h-5 text-gray-500" />
                      <span>Năm sản xuất: <span className="font-medium">{car.year}</span></span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Fuel className="w-5 h-5 text-gray-500" />
                      <span>Nhiên liệu: <span className="font-medium">{car.fuel}</span></span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Palette className="w-5 h-5 text-gray-500" />
                      <span>Màu sắc: <span className="font-medium">{car.color}</span></span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Gauge className="w-5 h-5 text-gray-500" />
                      <span>Số km: <span className="font-medium">{car.mileage.toLocaleString()} km</span></span>
                    </div>
                    <div className="flex items-center gap-2">
                      <MapPin className="w-5 h-5 text-gray-500" />
                      <span>Hộp số: <span className="font-medium">{car.transmission === 'automatic' ? 'Tự động' : 'Số sàn'}</span></span>
                    </div>
                    <div className="flex items-center gap-2 col-span-1 sm:col-span-2">
                      <PackageCheck className="w-5 h-5 text-gray-500" />
                      <span>Trạng thái: <span className={`font-medium ${car.status === 'available' ? 'text-green-600' : 'text-red-600'}`}>
                        {car.status === 'available' ? 'Còn hàng' : car.status === 'sold' ? 'Đã bán' : 'Đã đặt cọc'}
                      </span></span>
                    </div>
                    <div className="flex items-center gap-2 col-span-1 sm:col-span-2">
                      <Clock className="w-5 h-5 text-gray-500" />
                      <span>Cập nhật gần nhất: <span className="font-medium">{formatDate(car.updatedAt || car.createdAt)}</span></span>
                    </div>
                  </div>

                  <h2 className="text-2xl font-bold text-gray-900 mb-3">Mô tả chi tiết</h2>
                  <p className="text-gray-700 leading-relaxed mb-6">
                    {car.description || 'Chưa có mô tả chi tiết cho chiếc xe này.'}
                  </p>

                  {car.features && car.features.length > 0 && (
                    <>
                      <h2 className="text-2xl font-bold text-gray-900 mb-3">Tính năng nổi bật</h2>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      >
                        <span className="flex items-center justify-center gap-2">
                          <DollarSign className="w-5 h-5" /> {car.status === 'available' ? 'Đặt mua ngay' : 'Hết hàng'}
                        </span>
                      </Button>
                      <Button
