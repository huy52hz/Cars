...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    if (!carId || !car) {
      setOrderMessage(
        "Không tìm thấy thông tin xe để đặt hàng. Vui lòng chọn lại xe."
      );
  ...
  const validateForm = () => {
    ...
    if (!customerInfo.phone.trim()) {
      ...
      // Simple phone number regex for 10 or 11 digits
  ...
  const handleSubmitOrder = (e: React.FormEvent) => {
    ...
    setOrderMessage(
      `Đơn hàng xe "${car.name}" của bạn đã được đặt thành công!`
    );
  ...
  if (!car) {
    return (
      <div className="text-center py-10">
        ...
        <p>
          Nếu trang không tự động chuyển hướng, vui lòng quay lại và chọn xe.
        </p>
        <Link
          href="/cars"
